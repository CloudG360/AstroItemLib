package io.cg360.secondmoon.astroitemlib.commands.shop;
...
public class CommandGiveTemplateItem implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        if(!u.isPresent()){
            AstroItemLib.getLogger().error(String.format("Player %s was not found when applying item '%s' x %s", pname.get(), itemplate.get(), quantity.get()) );
            throw new CommandException(Text.of("Player not found."));
        }
        if(!item.isPresent()){
            AstroItemLib.getLogger().error(String.format("Player %s didn't recieve the item '%s' x %s as it didn't exist.", pname.get(), itemplate.get(), quantity.get()) );
            throw new CommandException(Text.of("Item not found."));
        }

        for(int i = 0; i < quantity.get(); i++) { Utils.givePlayerItem(u.get().getUniqueId(), item.get().generateStack(1).createSnapshot()); }
